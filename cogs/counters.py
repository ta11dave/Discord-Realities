...
class CounterCMDs(commands.Cog):
    ...
    async def CustomCounter(self, ctx, *args):
        ...
        if len(args)<1: # list all CCs
            ...
            for each in mychar.cc:
                ...
                eachcounter["min"] = each.minimum
                eachcounter["max"] = each.maximum
    ...
    async def create(self, ctx, name, *args):
        ...
        # !cc Name -min 0 -max -5 -value 5 -desc "text" (technically in any order or omit)
        arglist = []
        for each in args:
